...
def as_markdown(ok: list[dict], errs: list[dict]) -> None:
    print("# Static Python dialect reference\n")
    print("Generated by `python dialect/run.py --markdown`. Every row below was")
    print("produced by running the example, so this file cannot drift from the")
    print(f"runtime it documents.\n")
    for rec in ok:
        print(f"## `{rec['module']}`\n")
        if rec["status"] != "ok":
            print(f"**BROKEN**: `{rec['error']}`\n")
            continue
        doc = rec["doc"]
        if doc:
            print(doc + "\n")
        ...
        for label, value in rec["rows"]:
            print(f"| {label} | `{value}` |")
        print()
    print("## Rejected at compile time\n")
    print("| snippet | what it tries | diagnostic |")
    print("|---|---|---|")
    for rec in errs:
        mark = rec["diagnostic"].replace("|", "\\|")
        print(f"| `{rec['snippet']}` | {rec['what']} | `{mark}` |")
